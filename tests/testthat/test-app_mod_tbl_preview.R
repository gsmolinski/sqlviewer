...
shiny_app_obj <- shinyApp(set_ui(), set_server(connection))
...
test_that("app displays correct output when queries are removed", {
  skip_on_cran()
  skip_if_not(interactive())
  clipr::write_clip("
--#test2
...
", allow_non_interactive = TRUE)
  app <- shinytest2::AppDriver$new(shiny_app_obj, name = "test-queries-removing")
  app$wait_for_value(output = "tbl_preview-tbl_test1", timeout = 10000)
  app$set_inputs(remove_query = "test1", allow_no_input_binding_ = TRUE, priority = "event")
  app$set_inputs(remove_query = "test2", allow_no_input_binding_ = TRUE, priority = "event")
  app$set_inputs(remove_query = "test4", allow_no_input_binding_ = TRUE, priority = "event")
  app$set_inputs(remove_query = "test5", allow_no_input_binding_ = TRUE, priority = "event")
  app$expect_values(output = TRUE)
  app$stop()
})
